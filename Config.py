...
Config = {
    ...
    'lr_GAN': 0.001,       # GAN的学习率
    'latent_dim' : 64 ,    # 隐变量的维度
    'n_critic': 5,          # 训练D的次数
    'clip_value':0.01,      # 裁减D的参数（不要随便改）

    'load_G_name' : 'my_G17600.pth',  # 加载G的模型
    'epochs_lstm': 20000 ,             # LSTM训练的轮数
    'batch_size_lstm': 4 ,           # LSTM的batch （和序列有关，不要随便改）
    'lr_lstm': 0.001,                # LSTM的学习率
    'J': 8,                          # 用几张去预测一张
    'hidden_size' : 128 ,            # lstm menmory层的维度
    'layer': 2,                      # lstm 叠加的层数
}
